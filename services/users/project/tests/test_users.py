...
class TestUserService(BaseTestCase):
    ...
    def test_add_user(self):
        ...
        with self.client:
            response = self.client.post(
                ...
                data=json.dumps({
                    'username': 'eray',
                    'email': 'me@eray-ozer.com'
                }),
                content_type='application/json'
            )
            data = json.loads(response.data.decode())
            self.assertEqual(response.status_code, 201)
            self.assertIn('me@eray-ozer.com was added', data['message'])
            self.assertIn('success', data['status'])

    def test_add_user_incvalid_json(self):
    ...
    def test_add_user_invalid_json_keys(self):
        """A new user is added to the database only with valid payload."""
        with self.client:
            response = self.client.post(
                '/users',
                data=json.dumps({
    ...
    def test_add_user_duplicate_email(self):
        """A new user is added to the database only if not exists already."""
        with self.client:
            self.client.post(
                '/users',
                data=json.dumps({
    ...
    def test_single_user(self):
        ...
        with self.client:
            ...
            data = json.loads(response.data.decode())
            self.assertEqual(response.status_code, 200)
            self.assertIn('eray', data['data']['username'])
            self.assertIn('me@eray-ozer.com', data['data']['email'])
            self.assertIn('success', data['status'])
    ...
    def test_main_with_users(self):
        """The main route shows the newly added user."""
